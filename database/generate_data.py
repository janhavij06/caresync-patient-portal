...
#
# IMPORTANT:
# - Doctor licences/emails are unique for every run.
# - Patient emails are unique for every run.
# - Bills are created only for completed appointments
#   that do not already have a bill.
#
...
# ─── UNIQUE RUN ID ──────────────────────────────────────────────────────────
# Creates a unique ID every time this script is executed.
# Example:
# 20260907123456
#
# This prevents duplicate doctor licences and email addresses
...
# We store the IDs so we can use them for appointments later.
...
for i in range(NUM_DOCTORS):
    ...
    # UNIQUE email for every script run
    email = f'doctor{i+1}_{RUN_ID}@caresync.in'

    # UNIQUE licence for every script run
    lic = f'MCI-{RUN_ID}-{i+1:04d}'
    ...
    # lastrowid gives us the auto-generated doctor ID
    doctor_ids.append(cursor.lastrowid)


# Save all doctor rows to the database
connection.commit()
...
for i in range(NUM_PATIENTS):
    ...
    # UNIQUE email for every script run
    email = f'patient{i+1}_{RUN_ID}@example.com'
...
for _ in range(NUM_APPOINTMENTS):
    ...
    # Random appointment date over the past 2 years
...
# This prevents:
#
# Duplicate entry '2187'
# for key 'billing.uq_bill_appointment'
#
# The LEFT JOIN checks whether a billing record already exists.
...
#
# but ONLY for completed appointments
# that don't already have a bill.
...
if len(completed_appointments) < NUM_BILLS:
    ...
    print(
        ...
        f'completed appointments without bills available.'
    )

    print(
        f'  Will create one bill per available '
        f'completed appointment instead of {NUM_BILLS}.'
    )
    ...
    # Randomly pick NUM_BILLS from the available completed appointments.
    #
    # random.sample ensures that the same appointment
    # is not selected twice during this run.
...
for (appt_id, p_id) in bills_to_create:
    ...
    # ─── PARTIALLY PAID ─────────────────────────────────────────────────────
        ...
        # Patient paid between 30% and 70%.
    ...
    # Bill date:
    # Same day to 2 days after appointment.
